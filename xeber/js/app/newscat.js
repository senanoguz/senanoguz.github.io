$(document).ready(function () {
    ...
    var posts = firebase.database().ref('/newscategory')
  
    $(".sendToFireBase").click(function () {
  
      var newscat = $("#bolme").val();
      var newscatlinki = $("#linki").val();
    ...
    posts.on('value', function (res) {
      ...
      Object.keys(data).map(key => {
        ...
        var td1 = $('<td>').html(`<textarea class='edit'> ${data[key].newscat} </textarea><button style='width:30%' class='btn btn-warning btn-block editBtn'>Edit</button>`)
        var tdlink = $("<td>").html(`<textarea class='edit'> ${data[key].newscatlinki} </textarea><button class='btn btn-warning btn-block editBtn'>Edit</button>`)
