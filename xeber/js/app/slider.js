$(document).ready(function () {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
      if (user) {
        ...
        alert("Admin Panele girmek selahiyyetine malik deyilsiniz")
        window.location.href = "login.html"
      }
    ...
    var posts = firebase.database().ref('/slider')
    ...
    $(".sendToFireBase").click(function () {
      ...
      var sl2 = $("#sl2").val();
      var sl3 = $("#sl3").val();
      var slidermovzu1 = $("#slidermovzu1").val();
      var slidermovzu2 = $("#slidermovzu2").val();
      var slidermovzu3 = $("#slidermovzu3").val();
      ...
      posts.push(
        {
            ...
            sl2:sl2,
            sl3:sl3,
            slidermovzu1:slidermovzu1,
            slidermovzu2:slidermovzu2,
            slidermovzu3:slidermovzu3,
    ...
    posts.on('value', function (res) {
      ...
      Object.keys(data).map(key => {
        ...
        var td1 = $('<td>').html(`<img src="${data[key].sl1}"style="margin-bottom:20px;"><textarea class='edit'> ${data[key].sl1} </textarea><br>
        <textarea>${data[key].slidermovzu1} </textarea>
        <button style='width:30%' class='btn btn-warning btn-block editBtn'>Edit</button>`)
        var tdlink = $("<td>").html(`<img src="${data[key].sl2}" style="margin-bottom:20px;"><textarea class='edit'> ${data[key].sl2} </textarea><br>
        <textarea>${data[key].slidermovzu2} </textarea>
        <button style='width:30%' class='btn btn-warning btn-block editBtn'>Edit</button>`)
        var td2 = $("<td>").html(`<img src="${data[key].sl3}" style="margin-bottom:20px;"><textarea class='edit'> ${data[key].sl3} </textarea><br>
        <textarea>${data[key].slidermovzu3} </textarea>
        <button style='width:30%' class='btn btn-warning btn-block editBtn'>Edit</button>`)
