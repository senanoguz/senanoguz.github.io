$(document).ready(function () {
  ...
  posts.on('value', function (res) {
    ...
    Object.keys(data).map(key => {
      ...
      <textarea>${data[key].reklamustlinki} </textarea>`)
      ...
      var tdlink = $("<td>").html(`<img src="${data[key].reklamsag}"style="margin-bottom:20px;"><textarea class='edit'> ${data[key].reklamsag} </textarea><br>
      <textarea>${data[key].reklamsaglinki} </textarea>
      <button style='width:30%' class='btn btn-warning btn-block editBtn'>Edit</button>`)
      var tdorta = $("<td>").html(`<img src="${data[key].reklamorta}"style="margin-bottom:20px;"><textarea class='edit'> ${data[key].reklamorta} </textarea><br>
